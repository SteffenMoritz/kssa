...
kssa_plot <- function(
  ...
  if (class(results[[1]]) == 'kssa.table' & class(results[[2]]) == 'kssa.table'){
    ...
    if (type == "complete"){
      if (all(check) == TRUE){
        plot <- ggplot(data = plot1, aes(x = actual.method, y = eval(parse(text = metric)), fill = start.method)) +
          geom_boxplot()+
      ...
      if (all(check) == TRUE){
        plot <- ggplot(data = plot2, aes(x = actual.method, y = eval(parse(text = paste0('mean_',metric))), fill = start.method)) +
